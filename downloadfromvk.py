...
class DownloadsPhoto:
    ...
    def downloads_photo_from_vk(self):
        ...
        bar = Bar('Downloading', max=len(res.json()['response']['items']))
        list_name_fills_by_likes = []
        list_name_fills_by_date = []
        for i in res.json()['response']['items']:
            list_name_fills_by_likes.append(i['likes']['count'])
            list_name_fills_by_date.append(i['date'])
            url_photo = i['sizes'][-1]['url']
            self.size = i['sizes'][-1]['type']
            new_list_name = []
            for i, char in enumerate(list_name_fills_by_likes):
                if char not in new_list_name:
                    new_list_name.append(char)
                else:
                    list_name_fills_by_likes[i] = list_name_fills_by_date[i]
            response = requests.get(url_photo)
            for name in list_name_fills_by_likes:
                continue
